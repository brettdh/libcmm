...
NonBlockingTestsBase::startReceiverNB(EndToEndTestsBase *base)
{
    ...
    base->data_sock = cmm_accept(base->listen_sock, 
                                 (struct sockaddr *)&addr,
                                 &addrlen);
    handle_error(base->data_sock < 0, "cmm_accept");
    fprintf(stderr, "Receiver accepted connection %d\n", base->data_sock);
    set_nonblocking(base->data_sock);
}

void
NonBlockingTestsBase::startSenderNB(EndToEndTestsBase *base)
{
    base->data_sock = cmm_socket(PF_INET, SOCK_STREAM, 0);
    handle_error(base->data_sock < 0, "cmm_socket");
    ...
    int rc = cmm_connect(base->data_sock, (struct sockaddr*)&addr, addrlen);
    handle_error(rc == -1 && errno != EINPROGRESS, 
                 "unexpected connect() failure");

    fprintf(stderr, "Sender connection is in progress.\n");
    sock_wait(base->data_sock, false);

    int err = 0;
    socklen_t len = sizeof(err);
    rc = cmm_getsockopt(base->data_sock, SOL_SOCKET, SO_ERROR, &err, &len);
...
NonBlockingTestsBase::testTransferNB(EndToEndTestsBase *base)
{
    ...
    if (base->isReceiver()) {
        ...
        int rc = cmm_read(base->data_sock, &data, sizeof(data), NULL);
        CPPUNIT_ASSERT_EQUAL_MESSAGE("First read returns data",
                                     (int)sizeof(data), rc);

        fprintf(stderr, "Receiver about to read %zd bytes\n", sizeof(data));
        rc = cmm_read(base->data_sock, &data, sizeof(data), NULL);
        int err = errno;
        ...
        rc = cmm_write(base->data_sock, &data, sizeof(data),
                       0, NULL, NULL);
        handle_error(rc != 4, "cmm_write");

        fprintf(stderr, "Receiver about to select\n");
        sock_wait(base->data_sock, true);
        ...
        int rc = cmm_write(base->data_sock, &data, sizeof(data),
                           0, NULL, NULL);
...
NonBlockingTestsBase::testFragmentationNB(EndToEndTestsBase *base)
{
    ...
    if (base->isReceiver()) {
        ...
        while (bytes_recvd < msglen) {
            int rc = cmm_read(base->data_sock, buf + bytes_recvd, 
        ...
        while (bytes_sent < msglen) {
            ...
            int rc = cmm_write(base->data_sock, msg + bytes_sent,
